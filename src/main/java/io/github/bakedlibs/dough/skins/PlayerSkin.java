...
/**
 * Version-compatible replacement for dough's {@code PlayerSkin}.
 * <p>
 * It wraps a {@link CustomGameProfile} (which no longer subclasses {@code GameProfile})
 * and exposes the same factory methods as the original dough API, so callers do not need
 * to be changed.
 *
 * @see CustomGameProfile
 * @see PlayerHead
 */
public class PlayerSkin {
    ...
    public static @Nonnull CompletableFuture<PlayerSkin> fromPlayerUUID(Plugin plugin, UUID uuid) {
        ...
        Logger logger = Logger.getLogger("Skins");
        ...
        plugin.getServer().getScheduler().runTaskAsynchronously(plugin, () -> {
            ...
            try {
                ...
                logger.log(Level.WARNING, "Could not fetch skin from Mojang sessions server for " + uuid, e);
                future.completeExceptionally(e);
            }
        });
